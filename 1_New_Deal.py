...
def _preload_deal(deal_id: int) -> bool:
    ...
    # Note: county comes from `appraiser_county` (set in 2b above) — we don't
    # duplicate it here since there's only one canonical county widget.
...
with st.expander("Closing Cost Detail (county, short sale, concessions, HOA)",
    ...
    # County is set via the existing dropdown in the Contract Info section
    # below (key='appraiser_county'). We read it here rather than creating a
    # second widget — a single canonical widget avoids duplicate-key errors
    # and keeps both the appraiser link + closing cost model in sync.
    # If it isn't set yet, fall back to Broward.
    county = (st.session_state.get("appraiser_county") or "").strip() or "Broward"
    # These counties customarily have the BUYER pay the owner's title policy
    # at BC. Any other FL county has the SELLER pay it.
    _buyer_pays_title_counties = {"Broward", "Miami-Dade", "Sarasota", "Collier"}
    _title_note = ("buyer pays owner's title (no BC surcharge)"
                    if county in _buyer_pays_title_counties
                    else "seller pays owner's title (+0.4% BC surcharge)")
    st.info(
        f"📍 **County: {county}** — {_title_note}. "
        "Change the county in the Contract Info section below."
    )

    cc1, cc2 = st.columns(2)
    bc_commission_pct_ui = cc1.number_input(
    ...
    buyer_pays_seller_closings = cc3.checkbox(
    ...
    seller_concession = cc4.number_input(
    ...
    hoa_capital_contrib = cc5.number_input(
    ...
    utility_escrow_estimate = cc6.number_input(
    ...
    short_sale_lien_estimate = cc7.number_input(
...
with st.expander("📄 Contract Info (seller party, folio, legal description)",
    ...
    # v24 note: this is the CANONICAL county widget. It powers three things:
    #  1. The appraiser lookup link below
    #  2. The contract's Property Description
    #  3. The v24 closing cost model (owner's-title split, situational fees)
    # Expanded county list vs. v23 so Sarasota + Collier (also buyer-pays-title
    # counties) are pickable, plus the rest of coastal + I-4 corridor FL.
    _county_choices = [
    ...
    appraiser_county = c2.selectbox(
        "County (for appraiser lookup link & closing cost model)",
        _county_choices,
        index=(_county_choices.index(_guessed) if _guessed in _county_choices else 0),
        key="appraiser_county",
        help="Drives the appraiser look-up link below, the contract's Property "
             "Description, AND the v24 closing-cost model. "
             "Broward/Miami-Dade/Sarasota/Collier are buyer-pays-title counties; "
             "everywhere else the seller (us at BC) pays the ~0.4% owner's title.",
    )
